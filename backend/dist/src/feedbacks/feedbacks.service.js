...
let FeedbacksService = class FeedbacksService {
    ...
    async create(userId, dto) {
        ...
        if (user.role !== 'FUNCIONARIO')
            throw new common_1.ForbiddenException('Apenas funcionários podem enviar feedback.');
        return this.prisma.feedback.create({
            data: { userId, title: dto.title, message: dto.message },
        });
    }
    getMyFeedbacks(userId) {
    ...
    findAll() {
        return this.prisma.feedback
            ...
            ...f,
            user: { ...f.user, level: (0, get_user_level_1.getUserLevel)(f.user.points) },
        })));
    }
};
